...
- `公众号复制`：长文模式可复制带样式的富文本，直接粘贴到公众号编辑器。
- `公众号草稿同步`：本地版可复用系统钥匙串中的公众号配置，将标题、封面、正文样式和插图同步到草稿箱，不会直接发布。
...
图文排版、历史记录和下载功能均在浏览器本地完成，可以直接打开 `index.html`，或在项目目录执行：

```bash
npm run open
```

需要同步到公众号草稿箱时，请启动本机服务：
...
7. 需要复用时点击 `转长文` 或 `转图文`。
8. 图文模式下可单张下载或批量下载。
9. 长文模式下可复制公众号富文本，或在本地版中确认标题和封面后同步到公众号草稿箱。
...
- `复制公众号格式`：适合手动粘贴到公众号编辑器；浏览器会同时写入富文本和纯文本。
- `同步草稿箱`：只在 `npm start` 启动的本地版可用，会调用本机已有的公众号同步服务。
- App Secret 只保存在 macOS 系统钥匙串，不会写入仓库、网页代码或传给浏览器。
- 同步只创建或更新草稿，不会直接发布文章。
- Vercel 在线版无法访问你电脑的钥匙串，因此在线版保留复制与长图下载，不开放本机草稿同步。

## 技术说明

- 主要编辑与导出能力为纯前端实现；公众号草稿同步由本机 `server.py` 安全转发。
- 图文卡片使用 Canvas 渲染。
