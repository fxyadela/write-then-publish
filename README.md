...
- 把单段视频制作成可 AirDrop 到 iPhone 的 Live Photo 发布包；
- 使用 Supabase 账号分别保存头像、昵称、图文和素材；
- 复制带样式的公众号富文本；
...
账号功能使用 Supabase Auth、Postgres 和 Storage。首次进入时会先检查现有登录会话：有效账号会直接打开云端工作区；没有会话时可选择游客模式或登录。未配置 Supabase 时，游客模式仍可正常编辑和导出，登录入口会明确显示配置提示。

1. 创建一个 Supabase 项目。
2. 在 Supabase 的 SQL Editor 中运行 [`supabase/schema.sql`](supabase/schema.sql)。脚本会创建用户资料、图文项目、头像和项目素材所需的表、存储桶及 RLS 隔离策略。
3. 打开 [`src/supabase-config.js`](src/supabase-config.js)，填写 Project URL 和 publishable key：
...
4. 在 Supabase Auth 的 URL Configuration 中加入实际站点地址和本地测试地址，例如 `https://fawen.fun` 与 `http://localhost:5173`。

publishable key 是供网页使用的公开客户端密钥；不要把 `service_role` 密钥放进本项目或发送给他人。

游客草稿只写入当前标签页的会话存储，刷新当前标签页时可以继续，关闭后不承诺保留；登录后的资料、图文和素材才会进入账号长期工作区。右下角账号入口在游客状态下只提供“登录并同步”，登录后才显示账号管理和退出登录。账号面板提供“导入游客 / 旧本机草稿”按钮，只有用户主动点击时才会复制到当前账号，避免切换账号时混入其他草稿。旧版本已经存在的本机草稿不会被删除。
...
- 长文：导出 PNG 长图；
- 长文：复制带内联样式的公众号富文本；
- Live Photo：按小红书 5 秒或公众号 3 秒生成 `JPG + MOV + .pvt`；
- Obsidian：写回 Markdown 与附件，或下载 Obsidian 导入包；
- 本机扩展：确认标题和封面后同步到公众号草稿箱。

## 两种工作区

首次进入会先选择“游客模式”或“登录工作区”，进入后再通过 4 步聚焦引导说明：新建内容、选择发布形式、连接并同步 Obsidian，以及预览导出。内置“图文卡片说明书”包含普通图片和可播放的实况示例，不再只是文字说明。
...
| 长文排版与主题 | ✅ | ✅ | ✅ |
| 游客会话内临时历史 | ✅ | ✅ | ✅ |
| Supabase 账号长期保存 | 配置后可用 | 配置后可用 | 配置后可用 |
| PNG / ZIP 导出 | ✅ | ✅ | ✅ |
| 公众号富文本复制 | ✅ | ✅ | ✅ |
| Obsidian 图片读取 | ✅ | ✅ | ✅ |
...
在左侧写图文时，点击视频图标上传 MP4、MOV 或 WebM。选择平台、画面比例并拖动裁剪框后，视频会作为一张“实况图片”直接插入正文，和普通图片一起参与分页与排版。新素材默认取视频开头 3/5 秒，封面取开始后约 0.2 秒。

右侧不再提供独立的“制作实况”流程：

- 普通页面点击下载，正常导出 PNG；
- 页面包含一段实况素材时，下载按钮会自动生成整张卡片的 Live Photo 发布包；
- 混合批量下载会先按页分流：普通页面保留为高清 PNG，实况页面保留为完整 `.pvt`；完成面板可以把整批内容直接下载，或一次加入 macOS 原生 AirDrop；
- 稳妥首版每页支持一段实况素材，多段实况需要拆到不同页面。
- 切换到长文时，实况区域继续静音循环播放；复制公众号富文本或导出静态长图时使用对应封面图。

首版支持：

- 单视频素材；
- 小红书 5 秒与微信公众号 3 秒两个发布版本；
- 按发布平台自动截取开头 3/5 秒，并自动生成封面帧；
- 支持自由、原视频、`1:1`、`4:3`、`16:9`、`3:4`、`9:16` 画面比例；
- 调整横向、纵向画面位置，避免主体被裁切；
- 在右侧卡片中直接播放实况区域，周围标题和正文保持原有排版；
- 将视频 Blob 保存在浏览器 IndexedDB，刷新或切换历史记录后可恢复；
- 输出 H.264 MOV、JPG 封面和经过配对校验的 `.pvt`；
- 自动保存到 `~/Downloads/写了就发/Live Photo/` 并在 Finder 中显示。
- 单张实况完成后可以直接在 Finder 中显示或打开 macOS 原生 AirDrop；批量导出完成后还可以打开本次整理好的电脑文件夹。

推荐发布路径：

```text
左侧插入实况 → 正常完成图文排版 → 右侧下载自动识别 → Finder → AirDrop 到 iPhone → 照片确认“实况” → 手机 App 发布
```

请把整个 `.pvt` 当作一个项目 AirDrop，不要把 JPG 和 MOV 分开发送。JPG 与 MOV 会同时保留在输出目录中，用于调试和备份。

这项能力需要 macOS 本地版，以及本机可用的 `ffmpeg`、`ffprobe` 和 `uvx`。首次生成会通过 `uvx` 准备 MIT 许可的 [`makelive`](https://github.com/RhetTbull/makelive) 0.7.0，用于写入 Apple Live Photo 配对元数据。在线版可以预览视频与裁切效果，但不会伪装成已经生成 `.pvt`。

视频处理遵循“裁切，不拉伸”：直接从原视频读取选定时间段和画面区域，不生成中间压缩文件；最终整张卡片必须编码为平台可识别的 H.264 MOV，因此无法做到文件层面的零重编码。当前使用高质量编码、Lanczos 缩放，并保留原始帧率（最高 60fps），尽量避免可见的二次损失。
...
macOS 可以直接双击项目里的 `启动写了就发.command`。请保留打开的终端窗口；它会启动 Live Photo 本地生成服务并打开页面。
