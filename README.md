...
> 一份内容，排成图文卡片或公众号长文；图片、实况、Obsidian 与导出都在同一个工作区完成。实况照片直接在浏览器里合成，生成过程不上传原视频。
...
- 实况在浏览器本地合成，生成过程不上传原视频，单张约 1.6 秒；
- 与 Obsidian 双向同步；
...
> 这里说的「不上传」只针对生成实况这件事。**登录后的云端草稿同步是另一条独立链路**：为了跨设备恢复项目，它会把项目素材（包括这段视频）备份到你自己的 Supabase 存储。不想备份视频就用游客模式。
...
- 生成实况时原视频不出设备；回退到云端生成时会私有签名上传，源视频在任务结束后删除，下载链接约 1 小时失效；
- 登录后的云端草稿同步会另行备份项目素材（含视频）到你自己的 Supabase，这与实况生成无关，游客模式下不发生；
